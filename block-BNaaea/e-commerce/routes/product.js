...
router.get('/:slug/like', auth.loggedInUser, async (req, res, next) => {
  ...
  try {
    ...
    if (productLiked.likeId.includes(req.session.userId)) {
      // promise.all;
      const dislikeProduct = await Product.findOneAndUpdate(
        { slug: slug },
        { $inc: { likes: -1 } },
        { new: true }
      );
      const removeId = await Product.findOneAndUpdate(
        { slug: slug },
        { $pull: { likeId: req.session.userId } }
      );
      const removeUserLikeId = await User.findOneAndUpdate(
        { slug: slug },
        { $pull: { likeId: id } }
      );
      res.redirect('/product/' + slug + '/show');
      ...
      const productIncLike = await Product.findOneAndUpdate(
        { slug: slug },
        { $inc: { likes: counter } }
      );
      const productLikeId = await Product.findOneAndUpdate(
        { slug: slug },
        { $push: { likeId: req.session.userId } }
      );
      var { _id } = productLikeId;
      const userPushLikeId = await User.findByIdAndUpdate(req.session.userId, {
        $push: { likeId: _id },
      });
      res.redirect('/product/' + slug + '/show');
    }
